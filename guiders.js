...
var guiders = (function ($) {
    ...
    guiders.createGuider = function (passedSettings) {
        ...
        if (myGuider.title && myGuider.title.length) {
    ...
    guiders.next = function (timeout, callback) {
        var currentGuider = guiders._guiders[guiders._currentGuiderID];
        if (typeof currentGuider === "undefined" || currentGuider == null) {
            return null;
        }
    ...
    guiders.removeAll = function () {
        guiders.hideAll();
        $(".guider").each(function (index, elem) {
            $(elem).remove();
        });
        guiders._guiders = {};
        guiders._lastCreatedGuiderID = null;
    ...
    guiders.show = function (id) {
        ...
        if (myGuider.overlay) {
            ...
            if (myGuider.highlight && typeof myGuider.highlight == "string") {
                guiders._highlightElement(myGuider.highlight);
            } else if (myGuider.highlight && myGuider.attachTo) {
        ...
        function next() {
            ...
            if (myGuider.attachTo && myGuider.autoFocusAttachTo && !isElementInViewport($(myGuider.attachTo).get(0))) {
                ...
                window.setTimeout(function () {
                    $('html, body').animate({
                        scrollTop: $(myGuider.attachTo).offset().top
                    }, myGuider.scrollSpeed);
                }, myGuider.scrollTimeout);
            }

            if (myGuider.autoFocus && !isElementInViewport(myGuider.elem.get(0)) && !hasScrolled) {
                // Sometimes the browser won't scroll if the person just clicked,
                // so let's do this in a setTimeout.
                setTimeout(guiders.scrollToCurrent, myGuider.scrollTimeout);
            }
